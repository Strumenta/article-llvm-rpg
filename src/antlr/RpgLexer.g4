...
DIR_EOL : [ ]* NEWLINE {setText(cpptrim(getText()));} -> type(EOL),popMode;
...
NEXT_ARRAY :  '**' {getCharPositionInLine()==2}? .*? EOS_EOL -> type(END_SOURCE) ;
EOS_Text : ~[\r\n]+ ;
...
KEYWORD_SQLTYPE : [sS][qQ][lL][tT][yY][pP][eE] {contains(modeStack,FIXED_DefSpec)}?;
...
FREE_DOT: '.';
FREE_NUMBER_CONT: NUMBER {modeStack.back()==FIXED_DefSpec}? -> pushMode(NumberContinuation),type(NUMBER);
...
FREE_CONTINUATION : '...'
	{modeStack.back()!=FIXED_CalcSpec && modeStack.back()!=FIXED_DefSpec}?
	WS* NEWLINE -> type(CONTINUATION);
C_FREE_CONTINUATION_DOTS : '...' {modeStack.back()==FIXED_CalcSpec}? WS* NEWLINE
	(WORD5 [cC] ~[*] '                            ') {setText("...");} -> type(CONTINUATION);
D_FREE_CONTINUATION_DOTS : '...' {modeStack.back()==FIXED_DefSpec}? WS* NEWLINE
	(WORD5 [dD] ~[*] '                            ') {setText("...");} -> type(CONTINUATION);
C_FREE_CONTINUATION: NEWLINE {modeStack.back()==FIXED_CalcSpec}?
	(
		(WORD5 ~[\r\n] [*] ~[\r\n]* NEWLINE) //Skip mid statement comments
	|	(WORD5 ~[\r\n] [ ]* NEWLINE) //Skip mid statement blanks
	)*
	 WORD5 [cC] ~[*] '                            ' -> skip;
D_FREE_CONTINUATION: NEWLINE {modeStack.back() == FIXED_DefSpec}?
	WORD5 [dD] ~[*] '                                    ' -> skip;
F_FREE_CONTINUATION: NEWLINE {modeStack.back() == FIXED_FileSpec}?
	WORD5 [fF] ~[*] '                                    ' -> skip;
...
C_FREE_NEWLINE: NEWLINE {modeStack.back()==FIXED_CalcSpec}? -> popMode,popMode;
O_FREE_NEWLINE: NEWLINE {modeStack.back()==FIXED_OutputSpec_PGMFIELD}? -> type(EOL),popMode,popMode,popMode;
D_FREE_NEWLINE: NEWLINE {modeStack.back() == FIXED_DefSpec}? -> type(EOL),popMode,popMode;
F_FREE_NEWLINE: NEWLINE {modeStack.back() == FIXED_FileSpec}? -> type(EOL),popMode,popMode;
FREE_NEWLINE:   NEWLINE {modeStack.back()!=FIXED_CalcSpec}? -> skip,popMode;
...
FIXED_FREE_STRING_CONTINUATION: ('+' [ ]* NEWLINE) 
   {contains(modeStack,FIXED_CalcSpec) || contains(modeStack,FIXED_DefSpec)
...
FREE_STRING_CONTINUATION: {!contains(modeStack,FIXED_CalcSpec)
     && !contains(modeStack,FIXED_DefSpec)
     && !contains(modeStack,FIXED_OutputSpec)}?
      '+' [ ]* NEWLINE '       ' [ ]* -> skip;
FREE_STRING_CONTINUATION_MINUS: {!contains(modeStack,FIXED_CalcSpec)
     && !contains(modeStack,FIXED_DefSpec)
     && !contains(modeStack,FIXED_OutputSpec)}?
      '-' [ ]* NEWLINE '       ' -> skip;
PlusOrMinus: [+-];
...
FIXED_FREE_STRING_CONTINUATION_Part2:  
   (
     WORD5 
     ( [cC] {contains(modeStack,FIXED_CalcSpec)}?
      | [dD] {contains(modeStack,FIXED_DefSpec)}?
      | [oO] {contains(modeStack,FIXED_OutputSpec)}?
     ) 
     ~[*] 
     ( '                            ' {contains(modeStack,FIXED_CalcSpec)}?
       | '                                    ' {contains(modeStack,FIXED_DefSpec)}?
       | '                                             ' {contains(modeStack,FIXED_OutputSpec)}?
     ) 
     ([ ]* {modeStack.back() == EatCommentLinesPlus}?
      | 
...
COMMENTS_TEXT : ~[\r\n]+ {setText(cpptrim(getText()));} -> channel(HIDDEN);
...
PS_NAME :  NAME5 NAME5 NAME5 {getCharPositionInLine()==21}? {setText(cpptrim(getText()));};
PS_CONTINUATION_NAME : [ ]* ~[\r\n ]+ PS_CONTINUATION {setText(getText().substr(0,getText().length()-3));} -> pushMode(CONTINUATION_ELIPSIS) ;
PS_CONTINUATION : '...' ;
...
CONTINUATION_NAME : {getCharPositionInLine()<21}? [ ]* NAMECHAR+ CONTINUATION {setText(cpptrim(getText().substr(0,getText().length()-3)));} -> pushMode(CONTINUATION_ELIPSIS) ;
CONTINUATION : '...' ;
NAME : NAME5 NAME5 NAME5 {getCharPositionInLine()==21}? {setText(cpptrim(getText()));};
...
CE_D_SPEC_FIXED : [dD] {modeStack.back()==FIXED_DefSpec && getCharPositionInLine()==6}? -> skip,popMode ;
CE_P_SPEC_FIXED : [pP] {modeStack.back()==FIXED_ProcedureSpec && getCharPositionInLine()==6}? -> skip,popMode ;
...
OS_WS : [ \t] {getCharPositionInLine()>80}? [ \t]* -> skip  ; // skip spaces, tabs, newlines
...
LastRecordIndicator: [lL][rR] -> popMode;// { if (this._input->LA(1) == 10 || this._input->LA(1) == 13) modeStack.push(DEFAULT_MODE); };
...
CSQL_END : 
	 [cC] '/' [Ee][nN][dD][-][Ee][Xx][Ee][Cc] WS ~[\r\n]* {setText(cpptrim(getText()));} -> popMode ;
CSQL_CONT: [cC ] '+' -> skip; 
