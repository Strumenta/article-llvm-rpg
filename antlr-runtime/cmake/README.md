...
To build and ref ANTLR4 static library to a target one may call:
...
It may also be a good idea to copy the runtime libraries (DLL, DYLIB or SO file) to the executable for it to run properly after build. i.e. To build and ref antlr4 shared library to a target one may call:
